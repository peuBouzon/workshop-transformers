...
# 2. Create a trainer
# vocab_size is how many merges you want it to learn
trainer = BpeTrainer(vocab_size=8000, special_tokens=["[UNK]", "[PAD]", "[CLS]", "[SEP]"])

# 3. Train the tokenizer on your text
bpe_tokenizer.train_from_iterator(text_corpus, trainer)

# 4. Save the trained tokenizer to a file
tokenizer_path = "medical-bpe-tokenizer.json"
bpe_tokenizer.save(tokenizer_path)

print(f"BPE tokenizer trained and saved to {tokenizer_path}")

# --- 3. Load the Trained BPE Tokenizer ---
tokenizer_path = "medical-bpe-tokenizer.json"
tokenizer = Tokenizer.from_file(tokenizer_path)

# The vocabulary size is now determined by the tokenizer
vocab_size = tokenizer.get_vocab_size()

# The encode/decode functions now use the tokenizer
...
state_dict = torch.load('minigpt-medical.pth', map_location=CONFIG['device'])
...
for iter in range(CONFIG['max_iters']):
    if iter % CONFIG['eval_interval'] == 0 or iter == CONFIG['max_iters'] - 1:
      ...
      if val_loss < min_loss:
        min_loss = val_loss
        torch.save(model.state_dict(), 'minigpt-medical.pth')
